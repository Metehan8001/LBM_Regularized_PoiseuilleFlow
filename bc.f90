...
subroutine inlet() ! Zou-He boundary condition for velocity inlet
	...
	do j=2,jed-1
		...
		f(1,i,j)=f(3,i,j)+2*rho(i,j)*vel(i,j,1)/3.0
		f(5,i,j)=f(7,i,j)-0.5*(f(2,i,j)-f(4,i,j))+rho(i,j)*vel(i,j,1)/6.0
		f(8,i,j)=f(6,i,j)+0.5*(f(2,i,j)-f(4,i,j))+rho(i,j)*vel(i,j,1)/6.0
	enddo
end subroutine
!----------------------------------------------------------------------------
...
subroutine outlet_pressure() ! pressure boundary condition
	...
	i=ied
	do j=1,jed
		vel(i,j,1)=(f(0,i,j)+f(2,i,j)+f(4,i,j)+2.0*(f(1,i,j)+f(5,i,j)+f(8,i,j)))/rho_out-1.0
		f(3,i,j)=f(1,i,j)-2*rho_out*vel(i,j,1)/3
		f(6,i,j)=f(8,i,j)-0.5*(f(2,i,j)-f(4,i,j))-rho_out*vel(i,j,1)/6.0
		f(7,i,j)=f(5,i,j)+0.5*(f(2,i,j)-f(4,i,j))-rho_out*vel(i,j,1)/6.0
	enddo
end subroutine
!----------------------------------------------------------------------------
subroutine bouncebk 
